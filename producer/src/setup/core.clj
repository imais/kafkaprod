...
(defn run-callback [map exception]
  (println "map = " map ", exception = " exception))

(defn run [throughput with-skew? kafka-hosts redis-host]
  (println "Running, emitting" throughput "tuples per second.")
  (println "with-skew? = " with-skew? ", kafka-hosts = " kafka-hosts ", redis-host = " redis-host)
...
(def cli-options
  [["-s" "--setup" "Set up for catchup-simulation-mode (or re-setup if the .txt files exist)"]
   ...
   ["-b" "--bytes" "Send bytes to kafka at a particular frequency. This is used for the real-time simulation. Frequency specified with other options."]
   ["-l" "--length COUNT" "Should be used with '-b'. This is the number of length of bytes to send."
    :default 0
